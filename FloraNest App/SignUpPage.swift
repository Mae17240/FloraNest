    ...
    var body: some View {
        NavigationView {
            ZStack {
                ...
                VStack(spacing: 20) {
                    ...
                    if viewModel.isLoading {
                        ProgressView()
                            ...
                            .scaleEffect(1.5) // Note: need to fix effect!
                    } else {
                        Button(action: {
                            Task {
                                await viewModel.registerUser() // activates the registration
                            }
                        }) {
                            Text("Sign Up")
                ...
                NavigationLink(
                    destination: AccountPage(userName: viewModel.name, lastScannedPlant: "no plant scanned yet"),
            ...
            .alert("Registration Successful", isPresented: $viewModel.registrationSuccess) {
                Button("OK") {
                    dismiss() // returns to the sign in page
                }
            } message: {
...
struct CustomTextField: View {
    ...
    var body: some View {
        TextField(placeholder, text: $text)
            ...
            .overlay(
                Text(placeholder)
                    .foregroundColor(.white.opacity(text.isEmpty ? 0.6 : 0))
                    .padding(.leading, 18),
                alignment: .leading
            )
    }
